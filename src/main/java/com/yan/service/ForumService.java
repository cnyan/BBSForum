...
public class ForumService {
    ...
    public void removeTopic(int topicId) {
        Topic topic = (Topic) topicDao.get(topicId);

        // 将论坛版块的主题帖数减1
        Board board = boardDao.get(topic.getBoardId()).get(0);
        board.setTopicNum(board.getTopicNum() - 1);
    ...
    public void addPost(Post post){
        ...
        Topic topic = topicDao.get(post.getTopic().getTopicId()).get(0);
    ...
    public void removePost(int postId){
        Post post = postDao.get(postId).get(0);
        postDao.remove(post);
    ...
    public void removeBoard(int boardId){
        Board board = boardDao.get(boardId).get(0);
    ...
    public void makeDigestTopic(int topicId){
        Topic topic = topicDao.get(topicId).get(0);
    ...
    public Board getBoardById(int boardId) {
        return boardDao.get(boardId).get(0);
    }
    ...
    public Topic getTopicByTopicId(int topicId) {
        return topicDao.get(topicId).get(0);
    }
    ...
    public Post getPostByPostId(int postId){
        return postDao.get(postId).get(0);
    }
